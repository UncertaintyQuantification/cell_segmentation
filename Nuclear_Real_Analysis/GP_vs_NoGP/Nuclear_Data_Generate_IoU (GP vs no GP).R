...
source("../../src/Modified_Functions_RGasp.R") #GP Segmentation
source("Segmentation_Functions_NoGP (no conflict function names).R") #No GP Segmentation
...
plot_box <- ggplot(plot_data, aes(x = as.factor(Threshold), y = AP, fill = Method)) +
  ...
  #geom_jitter(width = 0.2, alpha = 0.5, size = 2, aes(color = Method)) + #include points
  labs(x = "Threshold", 
