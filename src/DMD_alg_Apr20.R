...
DMD_alg <- function(output, method="ratio_sum", threshold=0.999, 
  ...
  phi = Y %*% V %*% D_inv %*% eigen_A_tilde$vectors[,1:non_zero_eigen_val_count] %*% diag(1/eigen_A_tilde$values[1:non_zero_eigen_val_count],non_zero_eigen_val_count,non_zero_eigen_val_count)
  phi_inv = ginv(phi)
  if(r>1){
    A_hat_DMD = phi%*% diag(eigen_A_tilde$values[1:non_zero_eigen_val_count]) %*% phi_inv
  }else{
